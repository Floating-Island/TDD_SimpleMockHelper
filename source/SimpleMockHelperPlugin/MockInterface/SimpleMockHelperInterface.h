...
{   \
    ...
    return GlobalMockHelperInstance.lock()->ExecuteMockMethod<ReturnType, methodTraits::class>(ClassMethodAddress, ##__VA_ARGS__);    \
...
{   \
    using methodTraits = SimpleMockHelper::MethodTraits<decltype(ClassMethodAddress)>;    \
    return MockHelperInstance.lock()->ExecuteMockMethod<ReturnType, methodTraits::class>(ClassMethodAddress, ##__VA_ARGS__);    \
}/* add here a \ if you put your #if TEST_FLAG like the comment above */
//put here your #endif //TEST_FLAG